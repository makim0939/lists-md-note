        ...
        private static async Task<(string digestToken, DateTime expiresOn)> GetRequestDigestAsync(string siteUrl, CookieContainer cookieContainer)
        {
            using (var handler = new HttpClientHandler())
            {
                ...
                using (var httpClient = new HttpClient(handler))
                {
                    ...
                    //This is my bug fix. When running Asynchronously, the httpClient was never returning a result
                    //The Task hung here forever waiting for a response. Oddly, when I checked the call using Fiddler
                    //The response came back more or less immediately. Changing to a Synchronous call fixed the problem
                    //Go figure
